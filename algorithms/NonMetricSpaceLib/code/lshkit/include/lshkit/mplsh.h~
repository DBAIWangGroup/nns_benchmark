...
public: 
    ...
    /**
      ...
      * @param scanner 
      */
    template <typename SCANNER> //void
    int query (Domain obj, unsigned T, SCANNER &scanner)
    {
		int c=0;
		int io=0;
        ...
        for (unsigned i = 0; i < Super::lshs_.size(); ++i) {
            ...
            for (unsigned j = 0; j < seq.size(); ++j) {
				...
				//compute io = SIZE_OF_BIN / page_size
				io += ceil(tmp*sizeof(unsigned)/4096);
            }
        }
		std::cout<<c<<" "<<c+io<<std::endl;
		return c;
    }
