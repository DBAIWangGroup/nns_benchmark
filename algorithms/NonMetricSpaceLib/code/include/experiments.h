...
public:
  ...
  static void Execute(bool LogInfo, unsigned ThreadTestQty, size_t TestSetId,
    ...
    vector<double>    MAP(MethQty);
    vector<double>    Rde(MethQty);
    vector<double>    Mrr(MethQty);
    vector<double>    Dcg(MethQty);
      ...
      */
      ...
      for (unsigned QueryPart = 0; QueryPart < ThreadTestQty; ++QueryPart) {
        ...
        for (size_t qi = 0; qi < params->queries.size(); ++qi) {
		  ...
		  MAP[MethNum]          += Eval.GetMAP();
		  Rde[MethNum]          += Eval.GetRde();
		  Mrr[MethNum]          += Eval.GetMrr();
		  Dcg[MethNum]          += Eval.GetDcg();
          ...
          ExpRes[MethNum]->AddMAP(TestSetId, Eval.GetMAP());
          ExpRes[MethNum]->AddRde(TestSetId, Eval.GetRde());
          ExpRes[MethNum]->AddMrr(TestSetId, Eval.GetMrr());
          ExpRes[MethNum]->AddDcg(TestSetId, Eval.GetDcg());
    ...
    for (auto it = IndexPtrs.begin(); it != IndexPtrs.end(); ++it) {
      ...
      MAP[MethNum]               /= static_cast<double>(numquery);
