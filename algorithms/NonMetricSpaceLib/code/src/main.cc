...
void OutData(bool DoAppend, const string& FilePrefix,
  ...
  //std::ofstream   OutFileData(FileNameData.c_str(),(DoAppend ? std::ios::app : (std::ios::trunc | std::ios::out)));
  ...
  //std::ofstream   OutFileRep(FileNameRep.c_str(), (DoAppend ? std::ios::app : (std::ios::trunc | std::ios::out)));
	std::ofstream   OutFileRep(FileNameRep.c_str(), std::ios::app );
...
)
{
  ...
  cerr<<"start init meta"<<config.GetKNN().size()<<" "<<MethodsDesc.size()<<endl;
  vector<vector<MetaAnalysis*>> ExpResRange(config.GetRange().size(),
                                               vector<MetaAnalysis*>(MethodsDesc.size()));
//store the performance result of  methods and knn 
  ...
  for (int TestSetId = 0; TestSetId < config.GetTestSetToRunQty(); ++TestSetId) {
    ...
    try {
      ...
      for (const auto& methElem: MethodsDesc) {
        ...
        TotalMemByMethod = bCreateNew ? vmsize_after - vmsize_before  : prevMemUsed;//+ data_size
        TimeUsed = bCreateNew ? TimeElapsed : prevTimeUsed;
		...
		else
	    {
		  ...
		  isNewIndex.push_back(bCreateNew);
          IndexPtrs.push_back(IndexPtrs.back());
		  for (size_t i = 0; i < config.GetKNN().size(); ++i) {
            MetaAnalysis* res = ExpResKNN[i][MethNum]; 
            res->SetMem(TestSetId, TotalMemByMethod);
		    res->SetIndexTime(TestSetId, TimeUsed);
