...
float L2SqrSIMD(const float* pVect1, const float* pVect2, size_t qty) { // modify SIMD
//#ifndef PORTABLE_SSE2
//#pragma message WARN("L2SqrSIMD<float>: SSE2 is not available, defaulting to pure C++ implementation!")
	//cout<<"L2SqrtSIMD non  "<<endl; 
//*
	float res1 = 0, diff1;
    for (size_t i = 0; i < qty; ++i) {
        diff1 = pVect1[i] - pVect2[i];
        res1 += diff1 * diff1;
    }
	
    return res1;//*/
/*#else
	//cout<<"L2SqrtSIMD  "<<endl;
    size_t qty4  = qty/4;
    size_t qty16 = qty/16;
...
#pragma message WARN("L2NormSIMD<double>: SSE2 is not available, defaulting to pure C++ implementation!")
	cerr<<"L2NormStandard double  "<<endl;    
	return L2NormStandard(pVect1, pVect2, qty);
	
#else
	cerr<<"L2NormSIMD double  "<<endl;
    size_t qty8 = qty/8;
...
T LPGenericDistanceOptim(const T* x, const T* y, const int length, const T p) {
  ...
  cerr<<"lp generic distance optim "<<endl;
  CHECK(p > 0);
