...
namespace kgraph {
    ...
    public:
		...
		virtual void get_hubs (char const *hubs_path,char const *knn_path) const = 0; // save to file
        /// Build the index
         ...
         */
        unsigned search (SearchOracle const &oracle, SearchParams const &params, float* d_init, unsigned *ids, SearchInfo *info = 0) const {
            //std::cout <<"h search \n";
            return search(oracle, params,d_init, ids, nullptr, info);// d_init, Matrix<float> const &d_init,
        }
        /// Online k-NN search.
        /**
         * Search results are guaranteed to be ranked in ascending order of distance.
         ...
         */
        virtual unsigned search (SearchOracle const &oracle, SearchParams const &params,float* d_init, unsigned *ids, float *dists, SearchInfo *info) const = 0; //,Matrix<float> const &d_init
        /// Constructor.
