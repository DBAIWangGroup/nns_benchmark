...
inline float AverageRecall (Matrix<unsigned> const &gs, Matrix<unsigned> const &result, unsigned K = 0) {
        if (K == 0) {
            K = result.dim();
        }
        BOOST_VERIFY(gs.dim() >= K);
        ...
        for (unsigned i = 0; i < result.size(); ++i) {
            ...
            // compare
            std::set<unsigned> gs_set;
			for(unsigned j=0;j<K;j++)
				gs_set.insert(gs_row[j]);
			for(unsigned j=0;j<K;j++)
				if(gs_set.find(re_row[j])!=gs_set.end())
					sum++;
        }
        return sum*1.0 /K / result.size();
	...
	inline int get_position(const unsigned* gs,float id, unsigned K)
	{
	...
	inline float compute_mean_average_precision (Matrix<unsigned> const &gs, Matrix<unsigned> const &result, unsigned K = 0) {
        if (K == 0) {
            K = gs.dim();
        }
        BOOST_VERIFY(gs.dim() >= K);
        ...
        for (unsigned i = 0; i < result.size(); ++i) {
			...
			for(unsigned j=0;j<K;j++)
			{
				...
				for(unsigned t=0;t<=j;t++)
					if(gs_set.find(re_row[t])!=gs_set.end())
					{
						count++;
					}
				rate+=count*(count-found_last)/(j+1);
				found_last=count;
			}
            sum += rate/K;
        }
