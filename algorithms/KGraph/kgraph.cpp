...
//#pragma omp parallel for
    ...
    public:
        ...
        virtual unsigned search (SearchOracle const &oracle, SearchParams const &params, float* d_init, unsigned *ids, float *dists, SearchInfo *pinfo) const {
            ...
            for (unsigned trial = 0; trial < params.T; ++trial) {
                ...
                if (L == 0) {   // generate random starting points
                    ...
                    BOOST_VERIFY(d_init); //ids
                    std::cout<<"set seed"<<endl;
                    for (unsigned l = 0; l < L; ++l) {
                        knn[l].id = d_init[l];  //ids
                    }
                ...
                while (k < L) {
                    ...
                    if (knn[k].flag) {
                        ...
                        for (unsigned m = 0; m < maxM; ++m) {
                            ...
                            ++n_comps;
							//*
                            if(params.P==1 && n_comps >= params.Maxchecks)
					        {
                              break;
                            }//*/
                ...
                if (results.empty()) {
                    ...
                    for (unsigned l = 0; l < L; ++l) {
                        ...
                        if (r < results.size()  && results.size() < (params.K + 1)) {
