...
//*
	int neig_counts[17]={100,200,500,1000,2000,3000,5000,7000,10000,15000,17000,20000,25000,30000,40000,50000,60000};
	std::ofstream out(report_file.c_str(),ios::app);
      ...
      {
      	searcher.GetNearestNeighbours(queries[i],neig_counts[j], &result[i],dataset);
      }
      ...
      for(int i = 0; i < queries_count; ++i) //queries_count
      {
		...
		rde += compute_relative_distance_error(k,dataset,queries[i],groundtruth[i],result[i]);
		map +=compute_mean_average_precision(k,groundtruth[i],result[i]);
      }

	  recall = recall/queries_count;
	  rde = rde/ queries_count ;
	  time= time/queries_count ;
	  map = map/queries_count ;

	  out.setf(ios::fixed);  
	  out << recall<<" "<< rde << " " << time<<" #N_"<<neig_counts[j]<<" "<<map<<" "<<endl;  
	  if(recall==1.0)
	  {
		break;
	  }
