...
int main(int argc, char** argv)
{
	...
	char file_name[100];
	sprintf(file_name,"/home/liwen/Downloads/Dataset/%s/data/dataset_randomquery.hdf5",file_path);
	...
	sprintf(result_path,"/home/liwen/Downloads/Dataset/%s/exp_fig/result/Auto/k=%d/flann_Auto_%.2fPrecision_%.2fBuild_%.0fMemory_randomquery.txt",file_path,nn,target_precision,build_weight,memory_weight);
    out.open(result_path);
    
	{
    
	char index_path[100];
	sprintf(index_path,"/home/liwen/Documents/LSH/flann-1.8.4-src/build/bin/auto/%s_Auto_indices_%.2fPrecision_%.2fBuild_%.0fMemory_randomquery",data_name,target_precision,build_weight,memory_weight); 
	Index<L2<float> > index(dataset, flann::SavedIndexParams(index_path));                                                                      
  

    int checks[15]= {100,300,500,700,1000,2000,3000,4000,5000,7000,8000,10000,15000,20000,30000}; //{100,200,300,500,700,1000,1500,2000,3000,5000,7000,10000,15000,20000,30000}; 
    for(int c=0;c<15;c++)
    {
