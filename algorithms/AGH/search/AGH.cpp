...
int main(int argc, char * argv[]) {
	const struct option longopts[] ={
	  ...
	  while (iarg != -1) {
	    iarg = getopt_long(argc, argv, "s:q:g:n:l:m:k:t:a:r:c:b:p:h",
	                       longopts, &index);
	  ...
	  //if(radis<0)
		//hammingDist(&B2,&B1,&Dist,2*k);
	  //else
	  	//hammingDist_ByRange(&B2,&B1,&Dist,radis);
	  ...
	  for (int i=0;i< maxChecks.size() ;i++)
	  {
		...
		// get results after re-ranking
	      ...
	      if(radis>=0)
	      {
	  	     ...
	  	     {
	  	  	     sum+=indices.at(i).size();//" "<< mrr<<" "<< nc << " "<< map << " "<< dcg <<
	  	     }
			 cout<<recall<<" "<<rde << " "<< search_time/nq << " #bit_"<< nbits<<" #radis_"<<radis<<" #Layer_"<<Layer<<" #t_"<<nearest_size<<" #maxChecks_"<<sum*1.0/indices.size()<< " "<< endl;
			 break;
	      }
		 else
		 	 cout<<recall<<" "<<rde << " "<< search_time/nq << " #bit_"<< nbits<<" #radis_"<<radis<<" #Layer_"<<Layer<<" #t_"<<nearest_size<<" #maxChecks_"<<maxChecks[i]<<" "<< endl;
	 }
